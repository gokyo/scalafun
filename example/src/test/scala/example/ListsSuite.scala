...
class ListsSuite extends FunSuite {
  ...
  // additional tests for 'max'
  test("max on Nil list") {
  ...
  test("max on zeros list") {
    assert(max(List(0, 0, 0)) === 0)
  }
  test("max on negative numbers list") {
    assert(max(List(-3, -7, -2)) === -2)
  }
  test("max on mixed list") {
    assert(max(List(1, 5, -88, 101, -907)) === 101)
  }
  test("max on duplicate elements list") {
    assert(max(List(1, 5, 1, 5, 4)) === 5)
  }

  // additional tests for 'sum'
  test("sum on Nil (empty list)") {
    assert(sum(Nil) === 0)
  }
  test("sum on empty list") {
    assert(sum(List()) === 0)
  }
  test("sum on zeros list") {
    assert(sum(List(0, 0, 0)) === 0)
  }
  test("sum on negative numbers list") {
    assert(sum(List(-3, -7, -2)) === -12)
  }
  test("sum on mixed list") {
    assert(sum(List(1, 5, -87, 101, -907)) === -887)
  }
  test("sum on duplicate elements list") {
    assert(sum(List(1, 5, 1, 5, 4)) === 16)
  }
}
